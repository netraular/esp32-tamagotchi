...
void GardenScreen::load() {
    ...
    lv_label_set_text(roomLabel, "Garden");
    lv_obj_set_style_text_font(roomLabel, &lv_font_montserrat_16, 0);
    lv_obj_align(roomLabel, LV_ALIGN_CENTER, 0, 0);

    Serial.println("GardenScreen cargada.");
}

void GardenScreen::update() {
    // No es necesario actualizar nada en esta pantalla
}

void GardenScreen::handleButtonEvent(const ButtonState& state, const ButtonChange& change) {
